...
public class PostServiceImpl implements PostService {
    ...
    @Override
    public PostResponse createClient(PostRequest postRequest) {
        postRequest.setClientid(UUID.randomUUID().toString());
        PostEntity clientEntity = modelMapper.map(postRequest, PostEntity.class);
        postRepository.save(clientEntity);
        return modelMapper.map(clientEntity, PostResponse.class);
    }
    ...
    @Override
    public PostResponse updateClient(PostRequest postRequest) {
        PostEntity clientEntity = modelMapper.map(postRequest, PostEntity.class);
        PostEntity dbEntity2 = postRepository.getPostEntityByClientid(postRequest.getClientid());
        clientEntity.setId(dbEntity2.getId());
        postRepository.save(clientEntity);
        return modelMapper.map(clientEntity, PostResponse.class);
    }
    ...
    @Override
    public PostResponse getClientById(String clientid) {
        PostEntity clientEntity = postRepository.getPostEntityByClientid(clientid);
        return modelMapper.map(clientEntity, PostResponse.class);
    }
    ...
    public List<PostResponse> getAllClient() {
        return postRepository.getClientEntityBy().stream()
                .map(client -> modelMapper.map(client, PostResponse.class))
                .collect(Collectors.toList());
    }
    ...
    public void deleteClientById(String clientid) {
        postRepository.deletePostEntityClientById(clientid);
    }
